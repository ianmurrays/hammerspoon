...
**Webview modules** (`scratchpad`, `gif_finder`) use `hs.webview` with embedded HTML/JS. Communication between Lua and JS uses `hs.webview.usercontent` message handlers (`window.webkit.messageHandlers.<name>.postMessage(...)`).
