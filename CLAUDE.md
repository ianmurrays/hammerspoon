...
| `mouse_grid` | Keyboard-driven mouse (Mouseless-style): full-screen hint grid + in-cell subgrid for click/right/double click, drag & drop, scroll mode; free mode (ijkl relative movement, timer-driven, idle auto-exit); context hint toast in all modes; canvas cached per screen, modal eventtaps while active | Tap left Cmd (grid) / Tap left Alt (free) |
